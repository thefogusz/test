...
```mermaid
flowchart TD
    subgraph Entry["AudienceWorkspace.tsx"]
        MODE_AI["โหมด AI\nค้นหาด้วย AI"]
        MODE_MANUAL["โหมด Manual\nค้นหาบัญชีตรงๆ"]
        MODE_RSS["โหมด Source\nไล่ดู RSS"]
    end

    subgraph Hook["useAudienceSearch.ts"]
        SEARCH["search()"]
        RANK["rankResults()"]
    end

    subgraph APIs["External / Services"]
        XSEARCH["TwitterService.ts\nsearchUsers()"]
        GROK["GrokService.ts\nrankExperts()"]
    end

    subgraph Results["Results + Action"]
        CARDS["Recommendation Cards\n(Thai reasoning)"]
        FALLBACK["Thai Fallback\n(ถ้า reasoning อ่อน)"]
        WATCHLIST["useWatchlist.ts\naddAccount()"]
        PERSIST["persistence/client.ts\nsave watchlist"]
    end
    ...
    GROK --> RANK
    RANK --> CARDS
    CARDS -- "reasoning อ่อนเกินไป" --> FALLBACK
    CARDS --> WATCHLIST
    WATCHLIST --> PERSIST

    style Entry fill:#1e293b,stroke:#334155
    style Hook fill:#1e1b4b,stroke:#3730a3
    style APIs fill:#1c1917,stroke:#78350f
    style Results fill:#052e16,stroke:#166534
```
