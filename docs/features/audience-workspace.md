...
```mermaid
flowchart LR
    subgraph WS["AudienceWorkspace.tsx"]
        AI[โหมด AI]
        MANUAL[โหมด Manual]
        RSS_M[โหมด Source]
    end

    subgraph Hook["useAudienceSearch"]
        SEARCH[search]
        RANK[rankResults]
    end

    subgraph Svcs["Services"]
        XSEARCH[TwitterService]
        GROK[GrokService]
    end
    ...
    AI & MANUAL & RSS_M --> SEARCH
    SEARCH --> XSEARCH & GROK
    GROK --> RANK --> CARDS
    CARDS --> WATCHLIST --> PERSIST

    style WS fill:#1e293b,stroke:#334155
    style Hook fill:#1e1b4b,stroke:#3730a3
    style Svcs fill:#1c1917,stroke:#78350f
    style Out fill:#052e16,stroke:#166534
```
