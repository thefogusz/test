...
- result card ด้านบนของผลกรองเป็น structured analysis card ที่มี headline, บทสรุปสั้น 1-2 บรรทัด, output badge, section label ตาม intent และ bullet synthesis พร้อม citation badge
- ปุ่ม copy ของ result card ต้องคัดลอกเป็น plain text ที่แชร์ต่อได้ทันที ไม่มี markdown heading, confidence label หรือ decision note ติดไปด้วย
