...
| `foro_x_seen_registry_v1` | X seen registry สำหรับประวัติการ hydrate/mark seen ของโพสต์ที่ fetch แล้ว |
| `foro_x_sync_checkpoints_v1` | checkpoint ของ X sync แยกตามขอบเขต list/handle พร้อม recovery เมื่อ checkpoint ล้ำหน้าข้อมูล upstream |
...
- X seen registry ต้องไม่ถูกใช้เป็นเงื่อนไขเดียวในการซ่อน candidate ใหม่ เพราะโพสต์ที่เคย fetch แต่ยังไม่ถูกเพิ่มเข้า feed ต้องยังมีโอกาสขึ้นการ์ดได้
